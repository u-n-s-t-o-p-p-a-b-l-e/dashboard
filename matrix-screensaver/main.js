...
function draw()
{
	...
	ctx.fillStyle = "rgba(0, "+ rn+20 +" , " + rn+90 + ")";
...
doc.on("mousemove", (e) => {
	mX = e.pageX
	mY = e,pageY
	letter.push([mX-20+mT(20, 0), mY+mR(20, 0), letters[mR(letters.length, 0)], mR(10, 8), 1, mR(6, 1), mR(20, 0)])
})
...
think = () => {
	...
	for (let i = 0; i < letter.length; i++) {
		sC == 2 ? letter ? letter[i][2] = letters[mR(letters.length, 0)]:
		letter ? letter[i][1]-= letter[i][5]:
...
animate = () => {
	...
	for (let i = 0; i < letter.length; i++) {
		dT(letter[i][0], letter[i][1], letter[i][2], letter[i][3], letter[i][4]) 
			let rH = mR(540, 10)
			let sH = mR(rH, 1)
			letter[i][6] == 2 ? dR(letter[i][0], letter[i][1]-sH, letter[i][3]/1.5, rH, "rgba(0, 255, 100, 0.05)")
